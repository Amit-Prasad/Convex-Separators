...
def get_dataset(name):
    ...
    if name == "covtype":
        data = pd.read_csv('../evaluation/data/covtype.data', header=None).sample(frac=1).values
    ...
    if name == "santander":
        df = pd.read_csv("../evaluation/data/santander_train.csv")
        ...
        data_x, data_x_test, data_y, data_y_test = train_test_split(data_x, data_y, test_size=0.30, random_state=1)
        # df = pd.read_csv("data/santander_test.csv")
        # data_y_test = df['target'].values
        # df.drop(['ID_code', 'target'], axis=1, inplace=True)
        # df.sample(frac=1)
        # data_x_test = df.to_numpy()
        # data_x_test = (data_x_test / np.amax(np.abs(data_x_test), axis=0))
        return data_x, data_y, data_x_test, data_y_test
    ...
    if name == "santander_sub":
        ...
        data_x, data_x_test, data_y, data_y_test = train_test_split(data_x, data_y, test_size=0.30, random_state=1)
        # df = pd.read_csv("../datasets/santander_test.csv")
        # data_y_test = df['target'].values
        # df.drop(['ID_code', 'target'], axis=1, inplace=True)
        # df.sample(frac=1)
        # data_x_test = df.to_numpy()
